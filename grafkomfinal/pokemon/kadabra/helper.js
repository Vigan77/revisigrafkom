...
function generateBezierWhisker(segments, radius, color) {
    let vertices = [];
    let indices = [];

    // Titik Kontrol untuk Kumis Kadabra (Lengkungan S)
    const p0 = [0, 0, 0];        // Pangkal (di hidung)
    const p1 = [-0.5, -0.2, 0.3]; // Kontrol 1 (menjuntai ke bawah)
    const p2 = [-1.0, -0.1, 0.5]; // Kontrol 2 (mulai mendatar)
    const p3 = [-1.5, 0.2, 0.8];  // Ujung (melentur ke atas)

    // Helper untuk menghitung posisi titik pada kurva (t = 0 sampai 1)
    function getBezierPoint(t, p0, p1, p2, p3) {
        let cx = 3 * (p1[0] - p0[0]);
        let bx = 3 * (p2[0] - p1[0]) - cx;
        let ax = p3[0] - p0[0] - cx - bx;

        let cy = 3 * (p1[1] - p0[1]);
        let by = 3 * (p2[1] - p1[1]) - cy;
        let ay = p3[1] - p0[1] - cy - by;

        let cz = 3 * (p1[2] - p0[2]);
        let bz = 3 * (p2[2] - p1[2]) - cz;
        let az = p3[2] - p0[2] - cz - bz;

        let x = ax * Math.pow(t, 3) + bx * Math.pow(t, 2) + cx * t + p0[0];
        let y = ay * Math.pow(t, 3) + by * Math.pow(t, 2) + cy * t + p0[1];
        let z = az * Math.pow(t, 3) + bz * Math.pow(t, 2) + cz * t + p0[2];
        return [x, y, z];
    }

    // Membuat segmen-segmen lingkaran di sepanjang kurva (Sweeping)
    const circleSides = 8;
    for (let i = 0; i <= segments; i++) {
        let t = i / segments;
        let pos = getBezierPoint(t, p0, p1, p2, p3);

        for (let j = 0; j < circleSides; j++) {
            let angle = (j / circleSides) * Math.PI * 2;
            let x = pos[0];
            let y = pos[1] + Math.sin(angle) * radius;
            let z = pos[2] + Math.cos(angle) * radius;

            // Normal sederhana (bisa ditingkatkan dengan Frenet-Serret frame)
            vertices.push(x, y, z, color[0], color[1], color[2], 0, 1, 0);
        }
    }

    // Indexing untuk TRIANGLES
    for (let i = 0; i < segments; i++) {
        for (let j = 0; j < circleSides; j++) {
            let next_j = (j + 1) % circleSides;
            let curr = i * circleSides + j;
            let next = i * circleSides + next_j;
            let next_row = (i + 1) * circleSides + j;
            let next_row_next = (i + 1) * circleSides + next_j;

            indices.push(curr, next, next_row);
            indices.push(next, next_row_next, next_row);
        }
    }
